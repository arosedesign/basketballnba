...
if (!class_exists('REVIJA_BASE_FUNCTIONS')) {
	...
	class REVIJA_BASE_FUNCTIONS {
		...
		public function enqueue_styles_scripts() {
			...
			$prefix_name = 'revija'; //sanitize_file_name($mad_theme_data['prefix']);
		...
		public function enqueue_styles() {
			...
			wp_enqueue_style(REVIJA_PREFIX . 'style1', REVIJA_BASE_URI . 'css/style1.css');
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_script(REVIJA_PREFIX . 'jquery.hc-sticky', REVIJA_BASE_URI . 'js/jquery.hc-sticky.min.js', array('jquery'), '', true );
		...
		public function localize_script() {
			wp_localize_script('jquery', 'global', array(
				...
				'style_tweets' => mad_custom_get_option('style_tweets', 'carousel'),
				'primary_color' => mad_custom_get_option('styles-primary_color', '#ff680d'),
				'sticky_navigation' => mad_custom_get_option('sticky_navigation', 1)
			));
			
			
			wp_localize_script( 'newsletter-widget', 'global', array(
				'ajaxurl'       => admin_url( 'admin-ajax.php' ),
				'ajax_nonce' => wp_create_nonce('ajax-nonce')
			));
