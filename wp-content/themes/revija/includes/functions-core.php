...
if ( !function_exists('mad_blog_post_th_btn') ) {
	...
	function mad_blog_post_th_btn($id = 0, $post_content = '', $title = '', $radius = 14, $image_size='165*110', $count_label = 2) {
		...
		$colors_main = mad_custom_get_option('styles-primary_color');
...
if (!function_exists('mad_which_archive')) {
	...
	function mad_which_archive() {
		...
		$term_title = get_tax_meta($category_id,'ba_title_field_id');
		echo($term_title);
		?>
...
if (!function_exists('revija_maps_key_for_plugins')) {

	add_filter( 'script_loader_src', 'revija_maps_key_for_plugins', 10 , 99, 2 );

	function revija_maps_key_for_plugins ( $url, $handle  ) {

		$key = mad_custom_get_option( 'api-key-google-maps' );
