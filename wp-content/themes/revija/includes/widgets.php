...
if (!function_exists('mad_add_to_mailchimp_list')) {
	...
	function mad_add_to_mailchimp_list() {
		...
		if (isset($data->error)) {
			...
			$result['text'] = esc_html__($data->error, 'revija');
...
if (!class_exists('mad_widget_social_links')) {
	...
	class mad_widget_social_links extends WP_Widget {
		...
		function update($new_instance, $old_instance) {
			...
			$instance['type_display'] = $new_instance['type_display'];
			return $instance;
		}
	...
	class mad_widget_mailchimp extends WP_Widget {
		...
		function load_script() {
			if ( is_active_widget( false, false, 'widget-zn-mailchimp', true ) && ! is_admin() ) {
				...
				wp_enqueue_script( 'newsletter-widget', MAILCHIMP_URL . 'js/newsletter.js', array('jquery', 'jquery-form'), $this->version, true );
			}
		}
		...
		function update( $new_instance, $old_instance ) {
			...
			$instance['type_newsletter'] =  stripslashes($new_instance['type_newsletter']) ;
			return $instance;
		}
...
if (!class_exists('mad_widget_our_writers')) {
	...
	class mad_widget_our_writers extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			$random_author = empty($instance['random_author']) ? 'no' : $instance['random_author'];
		...
		function form($instance) {
			...
			$title = strip_tags($instance['title']);
			$random_author = strip_tags($instance['random_author']);
			...
			<p>
			<label for="<?php echo esc_attr($this->get_field_id('random_author')); ?>"><?php esc_html_e('Random author', 'revija'); ?></label>
			<select class="widefat" id="<?php echo esc_attr($this->get_field_id('random_author')); ?>" name="<?php echo esc_attr($this->get_field_name('random_author')); ?>">
				<option value="no"<?php selected( $instance['random_author'], 'no' ); ?>><?php esc_html_e( 'no', 'revija' ); ?></option>
				<option value="yes"<?php selected( $instance['random_author'], 'yes' ); ?>><?php esc_html_e( 'yes', 'revija' ); ?></option>
			</select>
			</p>
...
if (!class_exists('mad_widget_popular_categories')) {
	...
	class mad_widget_popular_categories extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			if( $categories ){
				...
				foreach( $categories as $cat ){
					...
					echo '<li><a title="' . sprintf( esc_html__( "%s", "revija" ), $cat->name ) . '" href="' . get_category_link( $cat->term_id ) . '">'. $cat->cat_name .'</a>';
					if( $count_posts == 'yes' ){
		...
		function form($instance) {
			...
			<p>
			<label for="<?php echo esc_attr($this->get_field_id('count_posts')); ?>"><?php esc_html_e('Show count posts', 'revija'); ?></label>
			<select class="widefat" id="<?php echo esc_attr($this->get_field_id('count_posts')); ?>" name="<?php echo esc_attr($this->get_field_name('count_posts')); ?>">
				<option value="yes"<?php selected( $instance['count_posts'], 'yes' ); ?>><?php esc_html_e( 'yes', 'revija' ); ?></option>
				<option value="no"<?php selected( $instance['count_posts'], 'no' ); ?>><?php esc_html_e( 'no', 'revija' ); ?></option>
			</select>
			</p>
