...
if (!class_exists('REVIJA_HTML_BUILD')) {

	class REVIJA_HTML_BUILD extends REVIJA_FRAMEWORK {
		...
		function textarea($element) {
			...
			$output .= esc_html($element['std']) . '</textarea>';
			return $output;
		}
		...
		function widget_positions ($element) {
			...
			<div class="meta-set">
				...
				<?php if (is_array($footer_row_widgets_array)): ?>
					...
					<div class="meta-columns-set">
						...
						<?php for ($i = 1; $i < $columns + 1; $i++):
							...
							<div class="mod-columns <?php if (!empty($col)) { echo "mod-grid-{$col}"; } ?> <?php echo esc_attr($css_class) ?>">
								<span><?php echo esc_html($letters[$i-1]); ?></span>
							</div><!--/ .mod-columns-->
					...
					<input id="<?php echo esc_attr($element['id']) ?>" type="hidden" class="data-widgets-hidden" data-columns="<?php echo key($footer_row_widgets_array); ?>" name="<?php echo esc_attr($element['id']) ?>" value='<?php echo esc_attr($element['std']) ?>' />
		...
		function page_header() {
			...
			<div class="mad-admin-bar">
				<a class="help" target="_blank" href="http://velikorodnov.com/online-documentation/flatastic/"><?php esc_html_e('Help', 'revija'); ?></a>
				<a class="logout" href="<?php echo wp_logout_url(esc_url(home_url())); ?>"><?php esc_html_e('Logout', 'revija'); ?></a>
			</div><!--/ .mad-admin-bar-->
			...
			<aside class="mad-admin-aside">
				...
				<div id="mad-admin-logo">
					...
					<?php if ($title): ?>
						<a class="mad-admin-link" href="<?php echo admin_url('admin.php?page=revija'); ?>">
		...
		function header_holder() {
			...
			<div class="mad-heading-holder">
				<div class="mad-heading">
					<h3 class="mad-heading-title"><?php echo strip_tags($title) . ' ' . esc_html__('Theme Options', 'revija'); ?></h3>
				</div><!--/ .mad-heading-->
		...
		function footer_holder() {
			...
			<li class="mad-footer-holder clearfix">
				<span>
					<?php esc_html_e('Copyright &copy; 2015.', 'revija') ?>
					...
					<?php esc_html_e('All rights reserved.', 'revija') ?>
				</span>
		...
		function reset_button() {
			return '<a href="#" class="mad_button_reset mad-icon-reset">' . esc_html__('Reset Options', 'revija') . '</a>';
		}

		function save_button() {
			return '<a href="#" class="mad_button_save mad-icon-save">' . esc_html__('Save All Changes', 'revija') . '</a>';
		}
			...
			<div class="import-wrap">
				<a class="mad_button_import" <?php echo (isset($element['path'])) ? 'data-source="'. $element['source']. '"' : ''; ?> <?php echo (isset($element['path'])) ? 'data-path="'. $element['path']. '"' : ''; ?> href="#">
					...
					<?php if (isset($element['image'])): ?>
						<img src="<?php echo REVIJA_BASE_URI . $element['image']; ?>" alt=""/>
					<?php endif; ?>

					<div class="import-overlay">
						<span class="overlay-button"><?php esc_html_e('Click import data', 'revija'); ?></span>
					</div><!--/ .import-overlay-->

				<span class="import-started">
					<span class="import-loading"></span>
					<strong><?php esc_html_e('Starting import.', 'revija') ?></strong><br>
					<?php esc_html_e('Please dont reload the page. It may take a few minutes...', 'revija') ?>
				</span><!--/ .import-started-->
				</a>
			</div><!--/ .import-wrap-->
		...
		function export_config_file() {
			...
			<a href="<?php echo admin_url('admin.php?page=revija&theme_settings_export=1&generate_file=1'); ?>" class="button button-hero mad_button_export">
				<?php esc_html_e('Export Theme Settings', 'revija'); ?>
			</a>
		</span><!--/ .export-wrap-->
		...
		function import_config_file() {
			...
			<span class="export-wrap">
				<a href="javascript:void(0)" data-url="<?php echo admin_url("admin-ajax.php") ?>" data-_wpnonce="<?php echo $nonce_import_config ?>" data-action="<?php echo esc_attr($this->actionImportConfigOptions) ?>" class="button button-hero uploader-config-button">
					<?php esc_html_e('Import Theme Settings', 'revija'); ?>
				</a>
			</span><!--/ .export-wrap-->
