<?php
if (!class_exists('REVIJA_DYNAMIC_STYLES')) {

	class REVIJA_DYNAMIC_STYLES {
		protected $rules;
		protected $output = "";
		public $link_output = "";
		...
		function create_styles() {
			...
			if (!isset($revija_config['styles'])) return;

			$this->rules = $revija_config['styles'];
				...
				foreach($this->rules as $rule) {
					...
					if (isset($rule['elements'])) {
						...
						if (isset($rule['values']) && is_array($rule['values'])) {
							foreach ($rule['values'] as $key => $value) {
								if (isset($key) && method_exists($this, $key) && $value != "") {
									$this->output .= wp_specialchars_decode( $this->$key($rule), ENT_QUOTES );
								}
							}
						}
					}
		...
		function link_google_webfonts() {
			...
			$this->link_output .= "<link rel='stylesheet' id='". REVIJA_PREFIX ."google-webfonts' href='//fonts.googleapis.com/css?family=" . $this->fontlist . "' type='text/css' media='all'/> \n";
		}
