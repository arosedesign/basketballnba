	...
	class REVIJA_ADMIN_PAGES extends REVIJA_FRAMEWORK {
		...
		function admin_menu() {
			...
			foreach ($this->globalObject->option_pages as $key => $data_set) {
				if ($key === 0) {
					$the_title = esc_html__('Theme Option','revija');
		...
		function create_page() {
			...
			if (!$check_token) {
				...
				echo '<div class="stm-admin-message">';
					esc_html_e('Please make sure you have purchased this theme with the account you registered current token', 'revija');
				echo '</div>';
