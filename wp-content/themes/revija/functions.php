...
define( 'REVIJA_THEME_VERSION', $theme_info->get( 'Version' ) );
...
define('STM_ITEM_NAME', 'Revija');
// Product Registration
if(is_admin()) {
	
	/*Connect Envato market plugin.*/
	if (!class_exists('Envato_Market')) {
		require_once(get_template_directory() . '/admin/includes/envato-market/envato-market.php');
	}
	require_once get_template_directory() . '/admin/includes/theme.php';
	require_once get_template_directory() . '/admin/includes/admin_screens.php';
...
if (!function_exists('revija_generate_styles')) {
	function revija_generate_styles() {
		...
		$prefix_name = 'revija'; //sanitize_file_name($globalObject->theme_data['prefix']);
