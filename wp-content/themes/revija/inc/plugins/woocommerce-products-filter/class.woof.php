...
if (!class_exists('REVIJA_WOOF')) {
	...
	class REVIJA_WOOF {
		...
		public function registerWidgets() {
			register_widget('REVIJA_WOOF_Widget');
		}
		...
		public function woocommerce_settings_tabs_array($tabs) {
			$tabs['woof'] = esc_html__('Products Filter', 'revija');
			return $tabs;
		}
		...
		public function add_enqueue_scripts() {
			if ( is_active_widget( false, false, 'widget-woof-filter', true ) && ! is_admin() ) {
				...
				wp_enqueue_script('woof', WOOF_LINK . 'js/woof-front.js', array(REVIJA_PREFIX . 'woocommerce-mod'), WC_VERSION, true);
				...
				if ( $matched_products_query ) {
					foreach ( $matched_products_query as $product ) {
						if ( $product->post_type == 'product' )
							$matched_products[] = $product->get_id();
		...
		public function get_options() {
			$options = array(
				array(
					'name' => esc_html__('Products Filter Options', 'revija'),
					'type' => 'title',
					'desc' => '',
					'id' => 'woof_general_settings'
				),
				array(
					'name' => esc_html__('Show count', 'revija'),
					'desc' => esc_html__('Show count of items near taxonomies terms on the front', 'revija'),
					'id' => 'woof_show_count',
					'type' => 'select',
					'class' => 'chosen_select',
					'css' => 'min-width:300px;',
					'options' => array(
						0 => esc_html__('No', 'revija'),
						1 => esc_html__('Yes', 'revija')
					),
					'default'  => 0,
					'desc_tip' => true
				),
				array(
					'name' => esc_html__('Show reset', 'revija'),
					'desc' => esc_html__('Show reset products filter', 'revija'),
					'id' => 'woof_show_reset',
					'type' => 'select',
					'class' => 'chosen_select',
					'css' => 'min-width:300px;',
		...
		public static function attributes_table( $type, $attribute, $values = array(), $echo = true ) {
			...
			if ('color' == $type) {
				if (!empty($terms)) {
					$return = sprintf( '<table><tr><th>%s</th><th>%s</th></tr>', esc_html__( 'Term', 'revija' ), esc_html__( 'Color', 'revija' ) );
