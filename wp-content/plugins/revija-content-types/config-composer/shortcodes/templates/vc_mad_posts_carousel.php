...
class WPBakeryShortCode_VC_mad_posts_carousel extends WPBakeryShortCode {
	...
	public function html() {
		...
		<div class="mad_post_carousel <?php echo $nav_show; ?> post_carousel_<?php echo $carousel_type; ?> <?php if ($carousel_type == 'type2' || $carousel_type == 'type3') { echo ' photo_gallery '; } if ($carousel_type == 'type1') { echo ' news_gallery news_gallery_var2 '; } ?>  " style="border-color:<?php echo $title_color; ?> !important; margin-top:<?php echo $gap; ?>px;" >

			<?php if (!empty($title)): ?>
			
				<?php 
				echo "<h3 class='section_title' style='color:". $title_color ." !important;' >";
				if($title != '') {
					echo $title;
				}
				echo "</h3>";
			...
			<div id="owl_custom_<?php echo $rand_id; ?>" class="owl-custom-<?php echo $columns; ?>" >
				...
				<?php while ( $entries->have_posts() ) : $entries->the_post(); ?>
					...
					<?php
						...
						$link = get_permalink();
						$id = get_the_ID();
						$post_title = mad_post_content_truncate(get_the_title($id), $title_count, " ", "…");
